...
1. `server-and-stack` — `server.js` 入口、构建栈、Next.js 配置
2. `env-vars` — `.env.example` 全字段解释与安全分级
3. `vercel-dnspod` — Vercel 项目设置、域名绑定、DNSPod 解析
4. `cos-and-cdn` — COS 桶结构、对象键命名、版本指针
5. `font-hosting` — 自托管 Google Fonts 的抓取与上传
6. `asset-pipeline` — 资源发布流水线（`assets:prepare` → `build` → `publish`）
7. `rss-sitemap-robots` — 动态 SEO 文件
8. `telemetry` — 遥测（默认关闭，按 env 启用）
9. `seo-and-security` — 站点元信息、revalidate secret、TOTP、access cookie、远端图床
10. `content-pipeline` — 外部 GitHub 内容仓库结构
